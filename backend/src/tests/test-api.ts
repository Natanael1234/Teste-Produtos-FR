...
let testeAPI = () => {
    ...
    it(`${ctx.numTeste} - Deve criar um produto`, async () => {
        // monta os dados do produto a ser criado
        let resProd = await ctx.criarProduto('teste01');        
    });

    it(`${ctx.numTeste} - Deve atualizar um produto`, async () => {
    ...
    it(`${ctx.numTeste} - Deve buscar os produtos`, async () => {
    ...
    it(`${ctx.numTeste} - Deve buscar os produtos para compra (ativos)`, async () => {
        // faz uma consulta por produtos ativos
        let resProdutos1 = await ctx.buscarProdutos(true);

        // cria um produto ativo.
        await ctx.criarProduto('teste_01_ativo', {status:'ativo'});

        // cria um produto inativo.
        await ctx.criarProduto('teste_01_inativo', {status:'inativo'});

        // faz uma segunda consulta por produtos ativos
        let resProdutos2 = await ctx.buscarProdutos(true);
        ...
        let possuiInativos = !!resProdutos2.produtos.find((produto:any)=>produto.status=='inativo');
        expect(possuiInativos).toBe(false);
    });

    it(`${ctx.numTeste} - Deve buscar produtos por id.`, async () => {
    ...
    it(`${ctx.numTeste} - Deve deletar um produto`, async () => {
        // cria um produto
    ...
    it(`${ctx.numTeste} - Deve buscar o carrinho`, async () => {
        let resCarrinho = await ctx.buscarCarrinho();       
    });

    it(`${ctx.numTeste} - Deve inserir produtos no carrinho`, async () => {
    ...
    it(`${ctx.numTeste} - Deve limpar o carrinho`, async () => {
        // cria produtos para inserir no carrinho
    ...
    it(`${ctx.numTeste} - Deve buscar pedidos.`, async () => {
        let resPedido = await ctx.buscaPedidos();
    });

    it(`${ctx.numTeste} - Deve finalizar um pedido.`, async () => {
        // esvazia o carrinho
        let resEsvaziar = await ctx.esvaziarCarrinho();
        // busca o carrinho
        let resCarrinho1 = await ctx.buscarCarrinho();
        // busca os pedidos
        let resPedidos1 = await ctx.buscaPedidos();
        // cria produtos
        ...
        let resPedido = await ctx.buscaPedido(resFinaliza.pedidoId);        
